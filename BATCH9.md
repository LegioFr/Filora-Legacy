...
Les revues indépendantes de la PR #77 ont ensuite confirmé six contournements réalistes de cette même cause racine :
...
Le sixième finding déclenche la règle de sortie convenue : ne plus ajouter des chemins protégés un par un, mais changer le modèle de preuve pour que la suite E2E teste l’artefact réellement produit par le build.
...
- `.github/workflows/playwright-e2e.yml` : installer les dépendances avec `npm ci --ignore-scripts`, construire une seule fois le `dist/` du SHA exact via le binaire Vite installé, vérifier les fichiers PWA essentiels, puis lancer Playwright contre ce même artefact ;
