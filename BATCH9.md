...
8. après séparation en deux runners, une PR ordinaire pouvait encore ajouter `npm-shrinkwrap.json`, prioritaire sur le lockfile, et y orienter une dépendance vers un paquet local exposant un faux binaire `playwright` sur le runner E2E neuf.
...
Le plancher de risque calculé par `tools/filora_guard.py` suit désormais la même autorité : après authentification de `--base-ref`, il lit le contrat et l’état au **SHA exact de base**, donne priorité aux chemins Critiques déclarés/structurels, conserve le niveau Sensible des `sensitive_paths` de base, admet seulement la surface ordinaire et le fichier Batch autorisé par l’état de base, et classe tout autre chemin Critique. `_changed_paths()` reste inchangé et inclut les deux côtés d’un renommage/copie, ce qui empêche un déplacement d’un ancien contrôle Critique vers un chemin ordinaire de faire baisser le risque.
...
- `tools/filora_guard.py` : calculer le plancher de risque du diff depuis le contrat/état **de base authentifiés**, avec Critique par défaut pour tout chemin non admis et précédence du niveau Critique sur l’allowlist ordinaire ;
- `tests/test_workflow_guard.py` : couvrir le contrat de base contre le candidat, l’état de base pour l’admission du fichier Batch, un renommage Critique vers `src/**`, la précédence Critique sur un chemin compatible ordinaire, un chemin inconnu et un diff mixte où le risque maximal doit gagner ;
...
- sur le HEAD `64ccc121d7f123ba8c5dd8c4388a7ef8152e33a0`, GitHub a de nouveau donné `guard` SUCCESS et `e2e` SUCCESS ; le workflow E2E a exécuté deux jobs distincts `build` puis `e2e`, et **39/39 tests Playwright** ont passé. Le diff réel comptait alors 10 fichiers, `DEVELOPMENT.md` s’étant ajouté dans le cadre de la règle opérationnelle propriétaire ;
- la contre-revue Claude du paquet ciblé sur `_changed_paths()` a confirmé que les statuts de renommage/copie ajoutent bien ancien et nouveau chemins et que la recherche du maximum de risque ne perd aucune information lorsqu’un chemin Critique est rencontré. Après cette contre-revue favorable, Mickaël a autorisé l’extension exacte à `tools/filora_guard.py` et `tests/test_workflow_guard.py`, avec mise à jour de la trace existante, portant le périmètre à 12 fichiers.
...
4. revue indépendante du SHA exact, incluant l’allowlist base-side, le plancher de risque base-side du guard et leurs probes/régressions ;
