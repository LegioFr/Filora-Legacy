...
- **Batch 7 : intégrer Playwright comme automatisation réelle des tests d’interface et de navigation**, sur plusieurs formats représentatifs (mobile, tablette, PC classique et grand écran/ultra-wide). L’objectif est de décharger Mickaël d’une grande partie des tests répétitifs en faisant vérifier automatiquement les boutons, menus déroulants, modales, formulaires, validations, messages d’erreur, navigation, rechargement/persistance visible, débordements horizontaux, accessibilité des actions et grille de stock, avec captures/traces utiles en cas d’échec. Ces scénarios automatisés doivent couvrir autant que raisonnable les mêmes parcours sur les différents formats, tout en laissant à Mickaël la validation humaine du rendu, de la compréhension et du confort réel d’utilisation. Playwright complète donc fortement la validation humaine sans la supprimer. **Évaluer également l’installation et l’usage de Graphify comme aide locale à la compréhension du code et à l’analyse d’impact pour les IA, sans en faire une source de vérité ni un mécanisme de preuve, et ne le conserver que si le gain pratique est réel.**
